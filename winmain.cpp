...
//			}
		...
		case ID_VM_MONITOR_SCANLINE:
			...
			break;
#endif
#ifdef USE_SUPERIMPOSE
		case ID_VM_MONITOR_SUPERIMPOSE:
			config.superimpose = !config.superimpose;
			if (emu) {
				emu->update_config();
			}
			// Fake superimpose
			// ここに処理を書くとconfigの読み込み時に更新されないが，所詮「なんちゃって」実装なので……
			{
				LONG lStyle = GetWindowLong(hWnd, GWL_EXSTYLE);
				if (config.superimpose) {
					lStyle |= WS_EX_LAYERED;
					SetWindowLong(hWnd, GWL_EXSTYLE, lStyle);
					SetLayeredWindowAttributes(hWnd, RGB(1, 1, 1), 127, LWA_COLORKEY);		// look at palette_pc (display.cpp)
				} else {
					lStyle &= ~WS_EX_LAYERED;
					SetWindowLong(hWnd, GWL_EXSTYLE, lStyle);
				}
...
#if defined(USE_MONITOR_TYPE) || defined(USE_SCANLINE) || defined(USE_SUPERIMPOSE)
...
#endif
#ifdef USE_SUPERIMPOSE
	CheckMenuItem(hMenu, ID_VM_MONITOR_SUPERIMPOSE, config.superimpose? MF_CHECKED : MF_UNCHECKED);
#endif
}
#endif
...
#endif
#if defined(USE_MONITOR_TYPE) || defined(USE_SCANLINE) || defined(USE_SUPERIMPOSE) || defined(USE_DIPSWITCH)
